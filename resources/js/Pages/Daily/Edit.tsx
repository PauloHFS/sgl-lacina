...
export default function Edit({ daily, usuarioProjetos }: Props) {
    const { data, setData, put, processing, errors } = useForm<FormData>({
        usuario_projeto_id: daily.usuario_projeto_id,
        data: daily.data,
        ontem: daily.ontem,
        observacoes: daily.observacoes || '',
        hoje: daily.hoje,
        carga_horaria: daily.carga_horaria,
    });

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('daily.update', daily.id));
    };
                ...
                <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
                    <div>
                        ...
                        <p className="text-base-content/60 mt-0.5 text-sm">
                            Atualize seu registro de progresso
                        </p>
                    </div>
                    <Link
                        href={route('daily.show', daily.id)}
